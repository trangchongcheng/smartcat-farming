...
const account = {
  ...
  ids: [1880442457], // Nhập id của mèo anh em
};
/* Nhập danh sách id mèo cần accept playdate */
const microchipIds = [
  1136251290, 2639936330, 1257186730, 1357485611, 1153160109, 3154248340,
  3072345157, 3596697853, 3111802887, 3597153910, 1806417778,
];
...
async function main() {
  ...
  try {
    let write = await contract.acceptPlayDate(currentCat, currentMicro);
    console.log(
      `${
        microIndex + 1
      }: Hẹn hò thành công từ mèo ${currentCat} tới mèo ${currentMicro}, tx:  ${
        write.hash
      } \n`
    );
    ++microIndex;
    ...
    console.log(
      `${
        microIndex + 1
      }: Lỗi rồi, hẹn hò không thành công từ mèo ${currentCat} tới mèo ${currentMicro} , đang chuyển qua con khác, nguyên nhân lỗi:
      ${error.shortMessage} \n`
    );
    ++microIndex;
  }
